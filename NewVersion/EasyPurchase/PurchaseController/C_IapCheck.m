...
- (void)dealloc
{
    ...
    self.pStr_checkingUrl = nil;
    self.pStr_receipt = nil;
    self.pData_recv = nil;
}
...
- (void)sendrequest
{
    ...
    NSMutableURLRequest *urlRequest = [[NSMutableURLRequest alloc] initWithURL:[NSURL URLWithString:self.pStr_checkingUrl] cachePolicy:NSURLRequestReloadIgnoringLocalCacheData timeoutInterval:IAP_RECEIPT_TIMEOUT];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    NSString *pStr_result = [[NSString alloc] initWithData:self.pData_recv encoding:NSUTF8StringEncoding];
